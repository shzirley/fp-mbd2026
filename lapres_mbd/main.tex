...
\section*{Pembagian Tugas dan Kontribusi}
\addcontentsline{toc}{section}{Pembagian Tugas dan Kontribusi}

Penyelesaian Final Project ini merupakan hasil kerja sama tim dengan rincian pembagian tugas dan kontribusi sebagai berikut:

\begin{itemize}
    \item \textbf{Maleka Ghaniya} (5025241189) : ERD, \textit{Trigger}, \textit{Procedure}, \textit{Backend}
    \item \textbf{Jorell Ramos Sinaga} (5025241202) : CDM, \textit{Indexing}, \textit{Frontend}
    \item \textbf{Angela Vania Sugiyono} (5025241226) : PDM, \textit{Query}, \textit{Function}, \textit{DevOps}
\end{itemize}
...
\begin{longtable}{|c|p{2.5cm}|p{3.5cm}|p{8.5cm}|p{7.0cm}|}
\caption{Kasus Query pada Sistem Basis Data CineTrack}
\label{tab:query_cases} \\
\hline
\textbf{No} & \textbf{Kasus} & \textbf{Kebutuhan Informasi} & \textbf{Aljabar Relasional} & \textbf{Penjelasan} \\
\hline
\endfirsthead
\hline
\textbf{No} & \textbf{Kasus} & \textbf{Kebutuhan Informasi} & \textbf{Aljabar Relasional} & \textbf{Penjelasan} \\
...
$\pi_{\texttt{judul,\ status,\ nama\_genre}} ( \sigma_{\texttt{status\_tayang}=\textit{`Now\ Showing'}}(\texttt{film}) \allowbreak \bowtie_{\substack{\texttt{film.id\_film =} \\ \texttt{film\_genre.film\_id\_film}}} \texttt{film\_genre} \allowbreak \bowtie_{\substack{\texttt{film\_genre.genre\_id\_genre =} \\ \texttt{genre.id\_genre}}} \texttt{genre} )$ &
Operasi \textit{selection} ($\sigma$) memilih baris pada relasi \texttt{film} dengan status "Now Showing". Hasilnya digabungkan (\textit{join} $\bowtie$) dengan relasi \texttt{film\_genre} dan \texttt{genre} berdasarkan Foreign Key masing-masing. Terakhir, dilakukan \textit{projection} ($\pi$) untuk menampilkan atribut judul, status tayang, dan nama genre. \\
\hline
...
$\pi_{\texttt{waktu,\ film,\ kelas,\ harga\_dasar,\ cabang}} ( \texttt{jadwal\_tayang} \allowbreak \bowtie_{\substack{\texttt{jadwal\_tayang.id\_jadwal =} \\ \texttt{jadwal\_tayang\_film.jadwal\_tayang\_id\_jadwal}}} \texttt{jadwal\_tayang\_film} \allowbreak \bowtie_{\substack{\texttt{jadwal\_tayang\_film.film\_id\_film =} \\ \texttt{film.id\_film}}} \texttt{film} \allowbreak \bowtie_{\substack{\texttt{jadwal\_tayang.studio\_id\_studio =} \\ \texttt{studio.id\_studio}}} \texttt{studio} \allowbreak \bowtie_{\substack{\texttt{studio.cabang\_id\_cabang =} \\ \texttt{cabang.id\_cabang}}} \texttt{cabang} )$ &
Relasi \texttt{jadwal\_tayang} digabungkan (\textit{join} $\bowtie$) secara berantai dengan relasi turunannya hingga \texttt{cabang} berdasarkan relasi Foreign Key terkait. Hasil gabungan tersebut diproyeksikan ($\pi$) untuk menampilkan waktu tayang, judul film, kelas studio, harga dasar, dan lokasi cabang. \\
\hline
...
$\pi_{\texttt{id\_transaksi,\ pelanggan,\ total,\ metode}} ( \sigma_{\texttt{total\_tagihan} > 200000}(\texttt{transaksi}) \allowbreak \bowtie_{\substack{\texttt{transaksi.pelanggan\_id\_pelanggan =} \\ \texttt{pelanggan.id\_pelanggan}}} \texttt{pelanggan} \allowbreak \bowtie_{\substack{\texttt{transaksi.pembayaran\_id\_pembayaran =} \\ \texttt{pembayaran.id\_pembayaran}}} \texttt{pembayaran} )$ &
Operasi \textit{selection} ($\sigma$) menyaring relasi \texttt{transaksi} yang memiliki total tagihan $>$ 200000. Relasi hasil saringan ini digabungkan ($\bowtie$) dengan \texttt{pelanggan} dan \texttt{pembayaran} melalui Foreign Key. Kemudian, \textit{projection} ($\pi$) menampilkan ID transaksi, nama pelanggan, total tagihan, dan metode pembayaran. \\
\hline
...
$\pi_{\texttt{kategori,\ produk,\ SUM(qty),\ SUM(subtotal)}} ( \texttt{produk\_kantin\_transaksi} \allowbreak \bowtie_{\substack{\texttt{produk\_kantin\_transaksi.produk\_kantin\_id\_produk =} \\ \texttt{produk\_kantin.id\_produk}}} \texttt{produk\_kantin} \allowbreak \bowtie_{\substack{\texttt{produk\_kantin.kategori\_id\_kategori =} \\ \texttt{kategori.id\_kategori}}} \texttt{kategori} )$ dengan $\gamma_{\texttt{kategori,\ produk}}$ &
Relasi \texttt{produk\_kantin\_transaksi} digabungkan ($\bowtie$) dengan \texttt{produk\_kantin} dan \texttt{kategori} menggunakan Foreign Key. Setelah itu, operasi agregasi ($\gamma$) menghitung total kuantitas (SUM qty) dan pendapatan (SUM subtotal) yang dikelompokkan per kategori dan nama produk, lalu hasilnya diproyeksikan ($\pi$). \\
\hline
...
Operasi \textit{set difference} ($-$) digunakan untuk memfilter data. \textit{Projection} ($\pi$) pertama mengambil semua pelanggan terdaftar, lalu dikurangi dengan hasil \textit{projection} pelanggan yang sudah ada di relasi \texttt{transaksi}. Sisa data yang dihasilkan adalah pelanggan yang belum pernah bertransaksi. \\
\hline
\end{longtable}
\end{landscape}
...
-- Contoh pemanggilan (Menampilkan total tiket untuk seluruh pelanggan):
SELECT 
    id_pelanggan, 
    nama_pelanggan, 
    GetTotalTiketByPelanggan(id_pelanggan) AS total_tiket_dibeli
FROM pelanggan;
\end{lstlisting}
...
\subsection{\textit{Function} 4: \texttt{CekStatusKursi}}

\textit{Function} keempat bertujuan untuk mengecek apakah sebuah kursi pada jadwal tertentu sudah terjual atau masih tersedia. \textit{Function} ini menerima dua parameter yaitu \texttt{p\_id\_jadwal} dan \texttt{p\_id\_kursi}, kemudian mengembalikan tipe data \texttt{VARCHAR} berupa teks \texttt{Tersedia} atau \texttt{Terjual}.

\textbf{Skenario:} Pada antarmuka aplikasi pemesanan tiket, sistem membutuhkan cara yang sangat cepat dan efisien untuk melakukan *rendering* peta status kursi bagi pelanggan. Dengan menggunakan \textit{function} ini, sistem \textit{frontend} hanya perlu melakukan satu pemanggilan sederhana tanpa harus melakukan kueri \texttt{JOIN} berulang kali secara kompleks di sisi \textit{backend}.

\begin{lstlisting}[style=sql, caption={\textit{Function} 4 --- \texttt{CekStatusKursi}}, label={lst:func4}]
DELIMITER $$
...
BEGIN
    ...
    IF v_count > 0 THEN RETURN 'Terjual';
...
-- Contoh pemanggilan:
SELECT CekStatusKursi('JD0001', 'KR0001') AS StatusKursi;
SELECT CekStatusKursi('JD0001', 'KR0002') AS StatusKursi;
\end{lstlisting}

Pada uji coba pertama, kita melihat implementasi \texttt{CekStatusKursi} untuk mengecek kursi yang sudah dibeli (\texttt{Terjual}), lalu membandingkannya dengan kursi yang kosong (\texttt{Tersedia}).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{function/Function4_Terjual.png}
    \caption{Uji Coba \textit{Function} 4 --- Kursi Terjual}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{function/function4_tersedia.png}
    \caption{Uji Coba \textit{Function} 4 --- Kursi Tersedia}
\end{figure}

\subsection{\textit{Function} 5: \texttt{CekDiskonMember}}

\textit{Function} kelima bertujuan untuk menunjang sistem pemberian diskon bagi pelanggan setia. \textit{Function} ini menerima \texttt{p\_id\_pelanggan} dan mengembalikan besaran persentase diskon dalam bentuk desimal (\texttt{DECIMAL(5,2)}).

\textbf{Skenario:} Bioskop ingin mengadakan program \textit{loyalty} (loyalitas pelanggan) di mana pelanggan yang telah melakukan transaksi 3 kali atau lebih berhak mendapatkan potongan harga sebesar 10\%. \textit{Function} ini akan secara otomatis menghitung jumlah transaksi historis pelanggan dari tabel \texttt{transaksi} dan mengembalikan nilai persentase diskon yang berhak didapatkan.

\begin{lstlisting}[style=sql, caption={\textit{Function} 5 --- \texttt{CekDiskonMember}}, label={lst:func5}]
DELIMITER $$
...
BEGIN
    ...
    IF v_jml_transaksi >= 3 THEN RETURN 10.00;
    ELSE RETURN 0.00;
    END IF;
END$$
DELIMITER ;
...
-- Contoh pemanggilan:
SELECT CekDiskonMember('PL0001') AS DiskonPersen;
SELECT CekDiskonMember('PL0008') AS DiskonPersen;
\end{lstlisting}

Pengujian untuk \textit{Function} 5 memvalidasi bahwa pelanggan dengan transaksi kurang dari 3 kali akan mendapatkan nilai diskon sebesar 0\%, sedangkan yang memenuhi syarat mendapatkan 10\%.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{function/function5_pelanggan1trx.png}
    \caption{Uji Coba \textit{Function} 5 --- Pelanggan 1 Transaksi (0\%)}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{function/function5_pelanggan3trx.png}
    \caption{Uji Coba \textit{Function} 5 --- Pelanggan 3 Transaksi (10\%)}
\end{figure}
...
\subsection{Kasus 4: \textit{BatalkanTransaksi}}
\textit{Procedure} keempat dibuat untuk menangani skenario operasional pembatalan transaksi oleh pelanggan (\textit{refund}). Alih-alih melakukan serangkaian kueri kompleks secara manual, \textit{procedure} ini akan secara otomatis mengembalikan stok produk kantin ke sistem, menghapus kepemilikan tiket yang terasosiasi agar kursi dapat dibeli kembali, dan mengubah status pembayaran menjadi 'Batal'.

\begin{lstlisting}[style=sql, caption={\textit{Procedure} 4 --- \texttt{BatalkanTransaksi}}, label={lst:proc4}]
DELIMITER $$
...
BEGIN
    ...
    DELETE FROM tiket WHERE transaksi_id_transaksi = p_id_transaksi;
    
    UPDATE pembayaran SET status_pembayaran = 'Batal'
    WHERE id_pembayaran = v_id_pembayaran;
END$$
DELIMITER ;

-- Contoh pemanggilan:
CALL BatalkanTransaksi('TX0001');
\end{lstlisting}

Saat prosedur ini dijalankan, perubahan status transaksi menjadi 'Batal' dapat diobservasi secara langsung pada sistem, membuktikan bahwa pengembalian stok dan pembebasan tiket sukses tereksekusi.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{procedure_image/procedure4.png}
    \caption{Uji Coba \textit{Procedure} 4 --- \texttt{BatalkanTransaksi}}
\end{figure}

\subsection{Kasus 5: \textit{UpdateStatusFilmHarian}}
\textit{Procedure} kelima berguna sebagai bentuk pemeliharaan rutin otomatis (\textit{housekeeping}). \textit{Procedure} ini dirancang untuk mengecek seluruh film yang saat ini masih berstatus 'Coming Soon'. Apabila ternyata jadwal tayangnya sudah dijadwalkan pada hari ini (atau terlewat), maka status penayangannya akan otomatis ter-\textit{update} menjadi 'Now Showing'. Hal ini sangat membantu meringankan pekerjaan repetitif para admin.

\begin{lstlisting}[style=sql, caption={\textit{Procedure} 5 --- \texttt{UpdateStatusFilmHarian}}, label={lst:proc5}]
DELIMITER $$
...
BEGIN
    ...
    WHERE f.status_tayang = 'Coming Soon'
      AND EXISTS (
          SELECT 1 FROM jadwal_tayang_film jtf
...
Gambar di bawah ini mendemonstrasikan status penayangan film yang secara otomatis bergeser dari 'Coming Soon' ke 'Now Showing' usai pemanggilan prosedur pemeliharaan harian ini.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{procedure_image/procedure5.png}
    \caption{Uji Coba \textit{Procedure} 5 --- \texttt{UpdateStatusFilmHarian}}
\end{figure}
...
\subsection{Kasus 4: Pencegahan Stok Kantin Minus}
Sebagai langkah melengkapi sistem inventaris kantin, kasus keempat ini berfokus pada validasi ketersediaan stok yang krusial. \textit{Trigger} diatur untuk mengeksekusi pengecekan \texttt{BEFORE INSERT} pada tabel \texttt{produk\_kantin\_transaksi}. Jika kuantitas pesanan yang diinputkan kasir melebihi sisa stok riil yang ada di gudang kantin, transaksi tersebut akan langsung digagalkan dengan memunculkan pesan \textit{error} kepada operator.

\begin{lstlisting}[style=sql, caption={\textit{Trigger} 4 --- \texttt{Pencegahan Stok Minus}}, label={lst:trg4}]
DELIMITER $$
...
BEGIN
    ...
    FROM produk_kantin WHERE id_produk = NEW.produk_kantin_id_produk;
...
Simulasi pemesanan stok di atas batas kuantitas yang tersedia memicu \textit{trigger} untuk membangkitkan \textit{error message} 'Gagal: Stok kantin tidak mencukupi untuk jumlah yang diminta!', mencegah kebocoran data negatif.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{trigger/Trigger4.png}
    \caption{Uji Coba \textit{Trigger} 4 --- Pencegahan Stok Minus}
\end{figure}

\subsection{Kasus 5: Pencegahan Jadwal Tayang Bentrok}
Kasus kelima diimplementasikan untuk menghindari potensi kelalaian staf manajerial saat menyusun jadwal penayangan film (\textit{human error}). \textit{Trigger} ini berjalan otomatis secara \texttt{BEFORE INSERT} pada tabel \texttt{jadwal\_tayang} dan akan mendeteksi apabila jadwal baru yang dimasukkan berdekatan (kurang dari durasi standar 2 jam) dengan penayangan film lain pada studio yang persis sama. Jika terjadi \textit{overlapping}, penjadwalan akan ditolak oleh basis data.

\begin{lstlisting}[style=sql, caption={\textit{Trigger} 5 --- \texttt{Pencegahan Jadwal Bentrok}}, label={lst:trg5}]
DELIMITER $$
...
Simulasi penambahan jadwal tayang yang saling \textit{overlapping} di bawah durasi 2 jam sukses ditolak oleh basis data dengan melemparkan \textit{error} khusus, menjaga konsistensi jadwal tayang studio.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{trigger/trigger5.png}
    \caption{Uji Coba \textit{Trigger} 5 --- Pencegahan Jadwal Bentrok}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{index/output_idx5.jpeg}
...
% SECTION 10: DEMO APLIKASI WEB
% ============================================================

\section{Demo Aplikasi Web CineTrack}

Demo web untuk mengimplementasikan fungsionalitas \textit{database} secara utuh dapat dilihat pada link berikut: \url{https://its.id/m/CineTrack}
...
\begin{enumerate}
    \item \textbf{Perancangan Basis Data:} Sistem Basis Data CineTrack telah berhasil dirancang melalui tahapan \textit{Conceptual Data Model} (CDM) dan \textit{Physical Data Model} (PDM), serta telah dinormalisasi hingga Bentuk Normal Ketiga (3NF) guna mencegah redundansi dan anomali data.

    \item \textbf{Implementasi Fungsionalitas:} Seluruh aturan bisnis operasional bioskop telah diimplementasikan dengan sukses melalui 5 \textit{Query Cases} untuk pelaporan, 3 \textit{SQL Functions} untuk kalkulasi data baca, 3 \textit{Stored Procedures} untuk modifikasi massal, dan 3 \textit{Triggers} untuk otomatisasi seperti pencegahan \textit{double-booking} kursi.

    \item \textbf{Evaluasi dan Optimasi Kinerja:} Pembuatan \textit{Index} pada kolom-kolom yang sering digunakan untuk \textit{filtering} (\texttt{WHERE}) dan pengurutan (\texttt{ORDER BY}) terbukti secara signifikan meningkatkan kecepatan eksekusi \textit{query}, yang divalidasi melalui hasil \texttt{EXPLAIN}.

    \item \textbf{Pengujian dan Integrasi:} Keseluruhan sistem basis data telah berhasil diuji dan diintegrasikan secara \textit{Full-Stack} ke dalam aplikasi web CineTrack, yang membuktikan bahwa arsitektur basis data yang dibangun \textit{solid}, responsif, dan siap digunakan untuk operasi bioskop berskala multi-cabang.
\end{enumerate}
